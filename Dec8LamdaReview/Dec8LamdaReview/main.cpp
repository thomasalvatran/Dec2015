...
int main() {
  ...
  {
    size_t c = count_if(col.begin(), col.end(), [multi](int x) {
      return !(x % multi);
    }); // capture multi
    cout << "Integers that are multiple of: " << c << endl;
  }
  {
    size_t c = count_if(col.begin(), col.end(),
                        [=](int x) { return !(x % multi); }); // capture multi
    cout << "Integers that are multiple of: " << c << endl;
  }
  {
    int sum = 0;
  ...
  } // end of scope so variable can be resused
  {
    int sum = 0;
    int *sum1 = &sum;
    for_each(col.begin(), col.end(), [&sum](int x) {
      sum += x;
  ...
  {
    int sum = 0;
    for_each(col.begin(), col.end(), [=](int x) mutable {
      sum += x;
    }); // capture sum by value is copy from sum
  ...
  }
  {
    ...
    c = count_if(col.begin(), col.end(),
                 [](int x)
                     -> bool // landa more then 1 line specify return type ->
                 {
                   bool odd = x % 2;
  ...
  { // call lambda to print
    double x = 12.12;
    cout << [&x]() -> double { return x * 2; }()
         << endl; // return type if many line in lambda
